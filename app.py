...
st.image('delivered_baby.jpeg', width=400)
...
def load_county_data(geojson_file_path):
    ...
    geo_data.drop(columns=['ck_date'], inplace=True, errors='ignore')
...
county_geo_data = load_county_data(county_geojson_path)

def generate_county_map(county=None):
    if county:
        county_data = county_geo_data[county_geo_data['County'] == county]
...
def county_main():
    ...
    if county_selection != 'All':
        folium_map_html = generate_county_map(county_selection)
    else:
        folium_map_html = generate_county_map()
